...
2. Users should implement proper permission control during installation and usage. It is recommended to refer to the [File Permission Reference](#file-permission-reference) for configuration.
...
Triton-Ascend supports source code compilation and installation. During compilation, it will download dependent third-party libraries and execute build shell scripts, generating temporary program files and compilation directories. Users can manage permissions on files within the source code directory as needed to reduce security risks.
