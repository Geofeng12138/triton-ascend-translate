...
Introduction: Perform a left shift on the tensor based on the given value.
...
| `_builder` | - | Reserved parameter, external calls not currently supported |
...
2. The right operand `other` only supports scalars, not tensors (i.e., `x << 2` is valid, while `x << y` (where `y` is a tensor) is not currently supported).


### 2.4 Usage


The following example implements a left shift operation on the three-dimensional tensors `x0` and `x1`:
