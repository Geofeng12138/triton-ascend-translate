...
Conclusion: The parameters `start` and `end` of `arange` must be constants, hence they are untyped, supporting value ranges corresponding to their types, up to `int32`. Hardware instructions also only support up to `int32`.
