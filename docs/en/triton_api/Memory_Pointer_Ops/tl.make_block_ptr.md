...
Introduction: Create a pointer to a tensor on GM
Prototype:
...
The result of `tl.make_block_ptr` does not allow arithmetic operations. To change the offset, you can do so by:


1. Call `make_block_ptr` again and modify the `offset` parameter:
...
- Ascend only allows expressing transpose semantics by adjusting the order of the `order` parameter, and cannot achieve transpose semantics by adjusting the order of the `stride` parameter.


- | Difference | Description | Solution |
  | -------------------------------------- | ------------------------------------------------------------ | ------------------------------ |
  | Generalization issue when used with branching and loop statements | Currently, `tl.make_block_ptr` may encounter compilation issues when used with complex loops and branching statements | Extensive generalization testing to expose issues, iterative resolution |


### 2.4 Usage


The following example implements the functionality of reading a tensor and transposing it using `tl.make_block_ptr`:
