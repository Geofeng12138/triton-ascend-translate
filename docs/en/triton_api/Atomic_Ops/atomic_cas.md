...
Introduction: Atomic compare and swap operation, compares the value of *pointer with cmp. If they are equal, updates *pointer to val; otherwise, *pointer remains unchanged.  
Prototype:
...
It can be called as a member function of a tensor, such as `x.atomic_cas(...)`, which is equivalent to `atomic_cas(x, ...)`.
...
| --------- | ---- | ----------- |
| `pointer` | `triton.PointerDType` | The memory location to operate on. If `*pointer == cmp`, update `*pointer` to `val` and write the computed result back to this memory location |
| `cmp` | `pointer.dtype.element_ty` | The value to compare with the target memory |
| `val` | `pointer.dtype.element_ty` | The target value for update |
...
Conclusion: Ascend lacks support for fp64 and bf16 compared to GPU.
...
| ---------- | ----------- |
| Data Type | Ascend lacks fp64 support compared to GPU (hardware limitation) |
| sem | The officially accepted values in the community are "acquire", "release", "acq_rel" (default, representing "ACQUIRE_RELEASE"), and "relaxed".<br>We only support "acq_rel". |
