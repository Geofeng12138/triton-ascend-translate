...
Introduction: Atomic exchange operation, performs an atomic exchange operation at the specified memory location.
Prototype:
...
It can be called as a member function of a tensor, such as `x.atomic_xchg(...)`, which is equivalent to `atomic_xchg(x, ...)`.
...
No special requirements, default 8-dimensional.
...
| ----------- | ----------- |
| sem | The community official configuration accepts values of "acquire", "release", "acq_rel" (default, representing "ACQUIRE_RELEASE"), and "relaxed".<br>We only support "acq_rel". |
...
The following example implements atomic AND calculation:
