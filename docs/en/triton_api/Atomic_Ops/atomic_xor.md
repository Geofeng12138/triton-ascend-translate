...
Introduction: Atomic logical XOR operation, performs logical XOR at the specified memory location  
Prototype:
...
Can be called as a member function of a tensor, such as `x.atomic_xor(...)`, which is equivalent to `atomic_xor(x, ...)`.
...
| `scope` | `str`, optional | The thread scope observing the effects of the atomic operation<br>Accepted values are "gpu" (default), "cta" (cooperative thread array, thread block), or "sys" (representing "SYSTEM")<br>We only support "gpu" |
...
Relative community capability is lacking and cannot be realized.


| Difference | Description |
| ----------- | ----------- |
| sem | The officially accepted values for community configuration are "acquire", "release", "acq_rel" (default, representing "ACQUIRE_RELEASE") and "relaxed".<br>We only support "acq_rel". |
