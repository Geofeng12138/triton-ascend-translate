...
Can be called as a member function of a tensor, such as `x.atomic_add(...)`, which is equivalent to `atomic_add(x, ...)`.
...
| ------------- | ---- | ----------- |
| `pointer` | `triton.PointerDType` | The memory location to operate on. The result of the *pointer + val computation is written back to this memory location. |
| `val` | `pointer.dtype.element_ty` | The value used in the atomic addition operation (right operand). |
| `mask` | `int1` or `tensor<int1>`, optional | Specifies the data range to prevent out-of-bounds access. |
| `sem` | `str`, optional | Specifies the memory semantics of the operation.<br>Officially accepted values from the community are "acquire", "release", "acq_rel" (default, representing "ACQUIRE_RELEASE"), and "relaxed".<br>We only support "acq_rel":<br>- acquire: After acquiring the lock, previous release operations are visible (equivalent to a "read" operation that blocks until the "latest" data, i.e., data released by other threads, can be read).<br>- release: All operations before releasing the lock are visible to threads that subsequently acquire the lock (equivalent to a "write" operation that "synchronizes" all previous write operations). |
...
| sem | The officially accepted values for the community are "acquire", "release", "acq_rel" (default, representing "ACQUIRE_RELEASE"), and "relaxed". We only support "acq_rel". |
| scope | The accepted values are "gpu", "cta", or "sys". We only support "gpu". |


### 2.4 Usage


The following example implements atomic addition calculation:
