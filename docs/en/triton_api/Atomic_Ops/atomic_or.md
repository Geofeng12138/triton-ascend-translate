...
Introduction: Atomic OR operation, performs a logical OR (|) operation at the specified memory location.  
Prototype:
...
Can be called as a member function of a tensor, such as `x.atomic_or(...)`, which is equivalent to `atomic_or(x, ...)`.
