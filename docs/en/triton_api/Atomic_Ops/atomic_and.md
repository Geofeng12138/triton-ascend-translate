...
Can be called as a member function of a tensor, such as `x.atomic_and(...)`, which is equivalent to `atomic_and(x, ...)`.
...
|------------|-------------|----------|
| sem | The accepted values for the official community configuration are "acquire", "release", "acq_rel" (default, representing "ACQUIRE_RELEASE"), and "relaxed".<br>We only support "acq_rel". | Pending development |
| scope | The accepted values are "gpu", "cta", or "sys".<br>We only support "gpu". | Pending development |
