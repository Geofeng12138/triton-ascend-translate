...
Introduction: Compute the element-wise softmax of x.
...
| `dim`         | `int`      | Specifies the dimension on which to compute softmax                         |
| `keep_dims`   | `bool`     | Controls whether to retain the shape of the original dimension after computation |
| `ieee_rounding` | `bool`   | Controls whether floating-point operations follow the rounding rules of the IEEE 754 standard |
...
The following example implements element-wise softmax on the input tensor `x`:
