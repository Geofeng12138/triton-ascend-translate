...
**Description:** Given a seed scalar and an offset block, returns a random block of type float32 in **U**(**0**,**1**).  
**Prototype:**
...
Return value:  
A random block of type `float32`, with the same shape as `offset`, and values uniformly distributed in the interval `[0.0, 1.0)`.
