...
Description: Given a seed scalar and an offset block, returns a random block of type int32.
...
If multiple streams of random numbers are needed, using `randint4x` may be faster than calling `randint` four times consecutively.
