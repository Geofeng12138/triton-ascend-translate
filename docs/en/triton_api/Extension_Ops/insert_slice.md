...
Introduction: Insert a tensor (sub-tensor) into another tensor at a specified position, i.e., insert one tensor into another according to the offset, size, and stride parameters specified by the operation.  
Prototype:
...
| `sub` | `tensor` | The sub-tensor to insert, whose shape must match the shape specified by the `sizes` parameter |
...
2. The lengths of `offsets`, `sizes`, and `strides` must be the same as the number of tensor dimensions.
