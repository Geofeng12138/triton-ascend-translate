...
Introduction: **Convert the index of a row-major matrix of size size_i × size_j into the index of a column-major matrix, grouped by every size_g rows.**
...
The following example converts row-major matrix indices to column-major matrix indices in groups of `size_g` rows each:
