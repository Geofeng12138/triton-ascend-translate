...
Introduction: **Compute the matrix product of two matrix blocks represented in a scaled format**
...
| ------ | ----------------------- |
| GPU    | Supports 2D to 3D tensors |
| Ascend | Supports 2D to 3D tensors |
...
3. Due to hardware alignment requirements, the broadcast multiplier for the scale matrix must be limited to at least 16.


4. The currently supported scaling matrix format is int8, while the community uses uint8.
