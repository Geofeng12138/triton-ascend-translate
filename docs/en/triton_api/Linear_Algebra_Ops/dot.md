...
Introduction: Performs matrix multiplication on two tensors. The tensors must be two-dimensional or three-dimensional and have consistent dimensions. For three-dimensional blocks, `tl.dot` performs batched matrix multiplication, where the first dimension of each block represents the batch dimension.  
Prototype:
...
| `acc` | `int32 float32` | Tensor storing accumulation results. If not None, the result is added to this tensor. `acc_dtype` supports {:code:`float16`, :code:`float32`, :code:`int32`} |
| `input_precision` | - | Available options for NVIDIA. Select the precision mode to enable Tensor Cores acceleration |
| `max_num_imprecise_acc` | `int` | Number of low-precision accumulations (currently Ascend does not support low-precision accumulation) |
...
- `out_dtype` comparison: GPU lacks support for int8 and FP16 types.


### 2.4 Usage


The following example implements matrix multiplication computation on input tensors `x_ptr, y_ptr`. Refer to ascend/examples/generalization_cases/test_matmul.py:
